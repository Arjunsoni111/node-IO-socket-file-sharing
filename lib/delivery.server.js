...
Delivery.prototype.connect = function () {
  ...
  this.socket.on('delivery.connect', function () {
    ...
    console.log("delivery.connsdfdsect");
...
Delivery.prototype.subscribe = function () {
  ...
  this.socket.on('send.start', function (filePackage) {
    //receive.start
    var filetype = path.extname(filePackage.name);
    if (filetype == ".xlsx") {
      ...
      _this.socket.emit('send.msg', { success: false, message: 'Please upload xlsx file only' });
    }
  ...
  this.socket.on('send.msg', function (msg) {
    _this.socket.emit('send.msg', msg);
  });
...
Delivery.prototype.sendMsg = function (msg) {
